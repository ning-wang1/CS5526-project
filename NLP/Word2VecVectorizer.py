...
class Word2VecSimple:
	...
	def transform(self, corpus):
		...
		for document in corpus:
			...
			#print('alpha',alphanumeric_document)
			#print(len(alphanumeric_document))
			doc_dictionary = dict()
			...
			for word in doc_dictionary:
				if word.lower() in self.vocabs or word in self.vocabs:
					...
					if word == '':
						original_case_word = '0'
						index_in_normalized_model = self.word_index_dict[original_case_word]
						model_to_append = self.normalized_model[index_in_normalized_model]
						words_factors_list_of_lists.append(model_to_append)
							
				
			if len(doc_dictionary)!= len(words_factors_list_of_lists):
				aaaa+=len(doc_dictionary)-len(words_factors_list_of_lists)
				print('not equal'+ str(len(doc_dictionary)-len(words_factors_list_of_lists)))
			# for word in self.model.wv.vocab:
		...
		print('vect',len(vectorization))
		w = csr_matrix(np.asarray(vectorization))
		print('transform',w.shape)
		# return normalize(w, norm='l1', axis=0)
		print(aaaa)
		return w

	def transformB(self, corpus):
		if self.model is None:
			print("model was not created. Please call fit method before transform. Exiting!")
			exit()
	...
	def get_doc2vec(self, raw_content):
		...
		try:
			...
			model = Word2Vec(sentences, min_count=self.min_count, size=self.size, window=self.window,sg=sg)
			size_vocab = 0
	...
	def get_doc2vec_all_corpus(self, corpus):
		...
		sentences = LineSentence("Output.txt", max_sentence_length=15)
		# print(sentences)
		# exit()
		# Ref: https://radimrehurek.com/gensim/models/word2vec.html
		# min_count = 2
